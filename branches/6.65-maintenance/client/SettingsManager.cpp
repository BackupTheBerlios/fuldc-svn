...
const string SettingsManager::settingTags[] =
{
	...
	"SpyIgnoreTTH", "OpenTransfers", "BoldTransfers",
	"SENTRY",
	// Int64
...
SettingsManager::SettingsManager()
{
	...
	setDefault(LOG_FORMAT_POST_DOWNLOAD, "%Y-%m-%d %H:%M: %[target]" + STRING(DOWNLOADED_FROM) + "%[userNI], %[fileSI] (%[fileSIchunk]), %[speed], %[time], %[fileTR]");
	setDefault(LOG_FORMAT_POST_UPLOAD, "%Y-%m-%d %H:%M: %[source]" + STRING(UPLOADED_TO) + "%[userNI], %[fileSI] (%[fileSIchunk]), %[speed], %[time], %[fileTR]");
	...
	setDefault(BOLD_TRANSFERS, true);
