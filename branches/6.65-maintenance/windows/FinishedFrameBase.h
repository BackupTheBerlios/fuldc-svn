...
public:
	...
	LRESULT onCreate(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled) {
		...
		WinUtil::SetIcon(m_hWnd, iconName);
	...
	LRESULT onGetList(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
		...
		if((i = ctrlList.GetNextItem(-1, LVNI_SELECTED)) != -1) {
			FinishedItem *fi = ctrlList.getItemData(i);
			if(fi) {
				User::Ptr u = ClientManager::getInstance()->getUser(Text::fromT(fi->getText(COLUMN_NICK)));
				if(u) {
					QueueManager::getInstance()->addList(u, QueueItem::FLAG_CLIENT_VIEW);
				} else {
					addStatusLine(TSTRING(USER_NOT_AVAILABLE));
				}
			} else {
				addStatusLine(TSTRING(USER_NOT_AVAILABLE));
			}
...
protected:
	...
	void addStatusLine(const tstring& aLine) {
		ctrlStatus.SetText(0, (Util::getShortTimeString() + _T(" ") + aLine).c_str());
	}
