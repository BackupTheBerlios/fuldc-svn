...
void SearchFrame::on(SearchManagerListener::SR, SearchResult* aResult) throw() {
	...
	if( (onlyFree && aResult->getFreeSlots() < 1) ||
		(aResult->getTTH() == NULL && (aResult->getType() != SearchResult::TYPE_DIRECTORY))
		)
	{
		filtered++;
		return;
...
void SearchFrame::initHubs() {
	...
	for(it = clients.begin(); it != endIt; ++it) {
		...
		onHubAdded(new HubInfo(Text::toT(client->getHubUrl()), Text::toT(client->getHubName()), client->isOp()));
	}
