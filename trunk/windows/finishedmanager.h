...
public:
	...
	FinishedItem(string const& aTarget, string const& aUser, CID const& aCid, string const& aHub, string const& aTTH,
		int64_t aSize, int64_t aChunkSize, int64_t aMSeconds, time_t aTime, 
		bool aCrc32 = false) : 
		size(aSize), chunkSize(aChunkSize),
		milliSeconds(aMSeconds), time(aTime), crc32Checked(aCrc32), cid(aCid)
	{ 
