...
ShareManager::ShareManager() : hits(0), listLen(0), bzListLen(0), bzXmlListLen(0),
 dirty(false), refreshDirs(false), update(false), listN(0), lFile(NULL), 
	...
	lastIncomingUpdate(GET_TICK()), refreshIncoming(false), refreshDir(false), 
	refreshInit(false)
{ 
...
void ShareManager::load(SimpleXML* aXml) {
	...
	if(aXml->findChild("Share")) {
		...
		while(aXml->findChild("Directory")) {
			tmp = aXml->getChildData();
			loadDirs.push_back(tmp);
			incomingDirs[tmp] = aXml->getBoolChildAttrib("IsIncomingDir");
		}
		aXml->stepOut();
	}
	dirty = true;
...
void ShareManager::save(SimpleXML* aXml) {
	...
	for(Directory::MapIter i = directories.begin(); i != directories.end(); ++i) {
		...
		aXml->addChildAttrib("IsIncomingDir", incomingDirs[i->first]);
	}
	aXml->stepOut();
}
...
ShareManager::Directory* ShareManager::addDirectoryFromXml(SimpleXML *xml, Directory *aParent, string & aName, string & aPath){
	...
	while (xml->findChild("File")) {
		...
		if(tth != NULL)
			tthIndex.insert(make_pair(tth, lastFileIter));
...
void ShareManager::refresh(bool dirs /* = false */, bool aUpdate /* = true */, bool block /* = false */, bool incoming /*=false*/, bool dir /* = false*/, bool init /* = false */) throw(ShareException) {
	update = aUpdate;
	refreshDirs = dirs;
	refreshDir = dir;
...
bool ShareManager::refresh(const string& dir){
	if (dir[dir.size() - 1] == '\\') {
		refreshDirName = Util::toLower(dir.substr(0, dir.size() - 1));
	} else {
		refreshDirName = Util::toLower(dir);
	}
	
	//check if it's a relative path, if so change it to an absolute path
	bool found = false;
	StringMapIter i = dirs.find(refreshDirName);
	if(i != dirs.end()){
		refreshDirName = i->second;
		found = true;
	} else {
		for(i = dirs.begin(); i != dirs.end(); ++i){
			if(Util::stricmp(Util::toLower(i->second), refreshDirName) == 0){
				refreshDirName = i->second;
				found = true;
				break;
			}
...
void ShareManager::setIncoming(const string & dir, bool incoming /* = true */){
	if (dir[dir.size() - 1] == '\\') {
		incomingDirs[dir.substr(0, dir.size()-1)] = incoming;
	} else {
		incomingDirs[dir] = incoming;
	}
}

bool ShareManager::isIncoming(const string & dir){
	if (dir[dir.size() - 1] == '\\') {
		return incomingDirs[dir.substr(0, dir.size()-1)];
	} else {
		return incomingDirs[dir];
	}
}

void ShareManager::removeIncoming(const string& dir){
	string tmp;
	if (dir[dir.size() - 1] == '\\') {
		tmp = Util::toLower(dir.substr(0, dir.size() - 1));
	} else {
		tmp = Util::toLower(dir);
	}

	StringBoolMap::iterator i = incomingDirs.begin();
	for(; i != incomingDirs.end(); ++i){
		if(i->first.compare(tmp) == 0){
			incomingDirs.erase(i);
			break;
		}
	}
}
