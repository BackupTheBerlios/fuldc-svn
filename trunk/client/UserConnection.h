...
public:
	...
	void get(const string& aFile, int64_t aResume) { send("$Get " + Text::utf8ToAcp(aFile) + "$" + Util::toString(aResume + 1) + '|'); }; 	// No acp - utf conversion here...
	...
	void uGetBlock(const string& aFile, int64_t aResume, int64_t aBytes, bool utf8) { send((utf8 ? "$UGetBlock " : "$GetBlock ") + Util::toString(aResume) + ' ' + Util::toString(aBytes) + ' ' + (utf8 ? aFile : Text::utf8ToAcp(aFile)) + '|'); }
